...
#'
#' @description Returns the fitness values of all individuals as a data.frame
#' with columns f1, ..., fo, where o is the number of objectives and column
#' \dQuote{gen} for generation.
#'
#' @param log [\code{ecr_logger}]\cr
#'   The log generated by \code{initLogger}.
#' @param trim [\code{logical(1)}]\cr
...
#' @return [\code{list}] List of populations.
#' @family logging
getPopulationFitness = function(log, trim = TRUE) {
  assertClass(log, "ecr_logger")
  assertFlag(trim)
  ...
  pops = lapply(pops, function(pop) {
    pop$fitness = toParetoDf(pop$fitness)
    return(pop)
  })
  reduceToSingleDataFrame(pops, what = "fitness", group.col.name = "gen")
}
